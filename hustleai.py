...
import stripe
import os
from utils import get_bottom_nav_html, load_json, save_json, get_ip, generate_hustles, generate_single_hustle, generate_checklist, extract_skills_from_pdf
...
# Navigation (sidebar for now, bottom nav at end)
pages = {
        ...
        if st.button("Generate My Hustles"):
            if final_skills:
                with st.spinner("Generating…"):
                    ideas = generate_hustles(final_skills, xai_key)
            ...
            if idx < len(lst):
                ...
                with col1:
                    if st.button("Swipe Left (Dislike)"):
                        new = generate_single_hustle(final_skills, xai_key)
                ...
                with col2:
                    if st.button("Swipe Right (Like)"):
                        ...
                        if 'user_email' in st.session_state:
                            ...
                            data = {"idea": lst[idx], "checklist": generate_checklist(lst[idx], xai_key)}
                            save_json(path, data)
...
# Monetization
    ...
    if st.button("Upgrade to Pro ($4.99/month)"):
        # Stripe checkout (expand as needed)
        try:
            session = stripe.checkout.Session.create(
                ...
                line_items=[{
                    'price_data': {
                        'currency': 'usd',
                        'product_data': {'name': 'HustleAI Pro'},
                        'unit_amount': 499,
                    },
                    'quantity': 1,
                }],
            ...
            st.write(f"Redirecting to Stripe...")
